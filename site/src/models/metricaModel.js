...
function coletarInfoDisco(idCaixa) {

    let instrucao = `
    SELECT c2.nome, mc.qtd_consumido, c2.qtd_maxima, c2.ponto_montagem, c2.sistema_arquivos  
    FROM MetricaComponente mc 
    JOIN Componente c2 ON c2.id = mc.componente_id 
    JOIN (
    SELECT componente_id, MAX(dt_metrica) AS ultima_data
    FROM MetricaComponente
    GROUP BY componente_id
)   ultima_metrica ON mc.componente_id = ultima_metrica.componente_id AND mc.dt_metrica = ultima_metrica.ultima_data
    WHERE c2.tipo = 'disco' AND c2.caixa_eletronico_id = ${idCaixa};`
    return database.executar(instrucao);
